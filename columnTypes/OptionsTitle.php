...
class OptionsTitle extends \RockFinder3\Column {
  public function applyTo($finder) {
    ...
    $finder->query->leftjoin("`fieldtype_options` AS `_options_{$this->alias}` ON `_options_{$this->alias}`.`option_id` = `{$this->tableAlias}`.`data` AND `_options_{$this->alias}`.`fields_id` = `_fields_{$this->name}`.`id`");
